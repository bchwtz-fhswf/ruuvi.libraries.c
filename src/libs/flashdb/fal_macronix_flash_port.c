...
int fal_mx_read(long offset, uint8_t *buf, size_t size)
{
    //LOGDf("Read @0x%x, size %d\r\n", offset, size);
    ...
    while (size>0){
      ...
      // If start and end adress are in different pages, the data is devided to multiple read commands
      if (start_page != end_page) {
        ...
        mx_read(offset, buf, start_page_data_length);
    ...
    if(err_code == RD_SUCCESS) {
      ...
      LOGDf("Error reading @0x%x, size %d\r\n", offset, size);
      return -1;
    }
}
...
int fal_mx_write(long offset, const uint8_t *buf, size_t size) {
  uint32_t start_page, start_page_end;
  uint32_t end_page, end_address;
  uint32_t end_address_copy, offset_copy;
  uint32_t start_page_data_length, data_length, rest_data_length;
  ...
  if(err_code == RD_SUCCESS) {
    ...
    LOGDf("Error writing @0x%x, size %d\r\n", offset, size);
    return -1;
  }
}

int fal_mx_erase(long offset, size_t size)
{    
    //LOGDf("Erase @0x%x, size %d\r\n", offset, size);
    rd_status_t err_code = RD_SUCCESS;
    err_code = mx_sector_erase(offset);
    if(err_code == RD_SUCCESS) {
      return size;
    } else {
      LOGDf("Error erasing @0x%x, size %d\r\n", offset, size);
      return -1;
    }
}
...
{    
    ...
    .ops        = {fal_mx_init, fal_mx_read, fal_mx_write, fal_mx_erase},
